...
function ScrollColumn({
  ...
  // Sync to external value changes (not triggered by user scroll)
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Top spacer – allows item 0 to centre */}
      ...
      {/* Bottom spacer – allows last item to centre */}
      <div style={{ height: SPACER, flexShrink: 0 }} />
    </div>
  );
}
...
export function TimeInput({
  ...
  const isFilled =
    mode === "single" ? Boolean(currentValue) : true; // range always shows times
  ...
  const minusColor = isDisabled ? "#99A1AF" : "#6A7282";
  const asteriskColor = isDisabled ? "#99a1af" : "#c10007";
  const asteriskSmall = isDisabled ? "#99a1af" : "#c10007";
      ...
      const floatLabel = required ? (
        <div className="flex gap-[2px] items-center shrink-0 w-full">
          ...
          <p
            ...
            style={{ color: asteriskSmall, fontFamily: FONT }}
          >
            *
          </p>
        </div>
      ...
      const valueRow =
        mode === "single" && currentValue ? (
          ...
          <div className="flex gap-[8px] items-center shrink-0 w-full">
            ...
            <Minus size={20} color={minusColor} weight="bold" />
            <p
              className="leading-[20px] not-italic shrink-0 text-[16px] whitespace-nowrap"
              style={{ color: valueColor, fontFamily: FONT }}
            >
  ...
  const triggerPadding = isTriggerFilled
    ? "py-[6px]"
    : "py-[12px]";
  const triggerBase = `relative flex gap-[8px] items-center rounded-[8px] ${bgClass} px-[14px] ${triggerPadding} w-full`;
  ...
  const triggerInner = (
    <>
      ...
      {renderContent()}
      <div
        className={`flex flex-row items-center shrink-0 ${isTriggerFilled ? "self-stretch" : ""}`}
      >
        <Clock
  ...
  const handleOpenChange = (o: boolean) => {
    if (!isDisabled) {
      if (o) {
        ...
        setDraftSingle(currentValue ?? { hour: 0, minute: 0 });
  ...
  const triggerButton = (
    <button
      type="button"
      disabled={isDisabled}
      className={`${triggerBase} text-left cursor-pointer disabled:cursor-default`}
    >
      {triggerInner}
    </button>
  );
          ...
          <Popover.Portal>
            <Popover.Content
              ...
              style={{
                ...
                fontFamily: FONT,
                minWidth: mode === "single" ? 327 : undefined,
              }}
